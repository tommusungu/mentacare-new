...
const HomeArticles = ({ isDark, showAll = false }) => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      setLoading(true)
      try {
        // Create a query against the articles collection
        const articlesQuery = query(
          collection(db, "articles"),
          orderBy("createdAt", "desc"), // Sort by newest first
          showAll ? null : limit(3), // Limit to 3 articles on home screen
        )
  ...
  const renderArticleItem = ({ item }) => (
    <TouchableRipple
      className={`mb-4 overflow-hidden ${isDark ? "" : "bg-white"}`}
  ...
  return (
    <FlatList
      ...
      contentContainerStyle={showAll ? { paddingBottom: 20 } : {}}
    />
  )
}
